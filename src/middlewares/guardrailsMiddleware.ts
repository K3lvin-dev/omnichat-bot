...
async function moderateContent(text: string): Promise<boolean> {
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error(
        `Error moderating content: ${error.response?.status} - ${error.response?.statusText}`,
      );
    } else {
...
async function isCinemaRelatedLLM(text: string): Promise<boolean> {
  try {
    ...
    if (!apiKey) {
      console.warn(
        'OPENAI_API_KEY is not set. Skipping cinema relation check.',
      );
      return true;
    }
    ...
    if (axios.isAxiosError(error)) {
      console.error(
        `Error checking cinema relation: ${error.response?.status} - ${error.response?.statusText}`,
      );
    } else {
      console.error('Unexpected error during cinema relation check:', error);
    }
    return true;
  }
}
